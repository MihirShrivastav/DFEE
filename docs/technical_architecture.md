...
- **Color Compression (`filmic_v3`):** `apply_color_compression` compresses high chroma via a soft shoulder (reduced colour dynamic range) and applies a bounded, chroma-gated neighbour-lean (red→orange, blue→cyan), hue-honest and neutral-preserving. Driven by `film_color_compression` (`0..200`, `100` = stock default; server-clamped) plus per-stock `compression.strength` / `compression.threshold` / `compression.crosstalk` YAML. Supersedes the retired M7-003 `palette_range` anchor pass (its request field remains an accepted no-op pending final removal). `filmic_v3` with `film_color_density=0` and `film_color_compression=0` equals `filmic_v2`.
- **Endpoints:**

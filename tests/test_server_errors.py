...
class TestServerRawFailureHandling(unittest.TestCase):
    ...
    def test_preview_clamps_film_contrast_and_forwards_adaptive(self):
        ...
        self.assertEqual(response.status_code, 200)
        payload = native_mock.call_args.args[0]
        self.assertEqual(payload["film_contrast"], 200.0)
        self.assertEqual(payload["highlight_rolloff"], 120.0)
    ...
    def test_export_accepts_filmic_v2_effect_pipeline_version(self):
        native_result = {
            "status": "ok",
            "output_path": "raw_files/example_dfee.png",
            "report_path": "raw_files/example_dfee_report.json",
            "format": "8-bit PNG",
            "engine": self._fake_engine_info(),
        }

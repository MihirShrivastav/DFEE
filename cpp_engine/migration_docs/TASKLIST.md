...
| M3-015 | done | Meter Auto Balanced placement from broad highlights | `ctest --test-dir cpp_engine/out/build/windows-msvc-vcpkg -C Release --output-on-failure` | `filmic_v3` now caps upward Auto Balanced exposure from `p95` rather than `p99`. Sparse speculars, small sky patches, and dappled light no longer cause an ordinary scene to be classified as high-key; the tone shoulder still protects those values. |

...
| M6-003 | in_progress | Replace grain with deterministic procedural/precomputed fields | Visual QA plus determinism test | `filmic_v2` now has native grain dispatch, density-aware modulation, initial stock-strength differentiation, and deterministic field caching. Richer profile fields and final visual/performance probe remain. |
...
| M6-003C | done | Implement density-aware grain modulation | Native synthetic exposure test | `filmic_v2` boosts lower-mid/shadow grain relative to smooth highlights through gamma-space exposure-density modulation. |
| M6-003D | in_progress | Add stock-family grain differentiation | Native synthetic stock test | Initial heuristics differentiate fine-grain vs high-speed-like behavior from strength/size/chroma; explicit optional profile fields remain under M6-003E. |
...
| M6-003H | in_progress | Run visual/performance acceptance probe | No-server preview timing plus image QA | First no-server timing probe is complete. Visual QA across representative RAWs and stock families remains before marking M6-003 complete. |

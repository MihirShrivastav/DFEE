...
| M6-003H | in_progress | Run visual/performance acceptance probe | No-server preview timing plus image QA | First no-server timing probe is complete. Visual QA caught blotchy v2 grain in smooth skies; renderer now uses band-limited clumping and a low-frequency artifact test. Visual QA across representative RAWs and stock families remains before marking M6-003 complete. |

...
| M6-013 | active | Rebuild RAW develop contract and Auto Balanced safety | RAW/TIFF corpus: toe/midtone/highlight/chroma metrics plus clipping regressions | The initial 1.28 neutral baseline is insufficient as a complete RAW developer. Preserve highlight headroom with a baseline shoulder now; then measure sensor clipping, camera colour, and developed tone separately. Do not use stock YAML, a global exposure boost, or a global saturation boost to hide RAW/TIFF divergence. |
| M6-014 | planned | Implement wide-gamut colour-managed input pipeline | `migration_docs/WIDE_GAMUT_RAW_PIPELINE_PLAN.md` phase gates and corpus tests | Adopt linear Rec.2020 working RGB only behind `wide_gamut_v1`. Add ICC/DNG input transforms, explicit output transforms, and colour-space-aware engine math. Preserve `srgb_legacy_v1` recipe playback. |
| M6-015 | planned | Classify RAW, linear DNG, and developed RGB by content | Mosaic/linear DNG, tagged/untagged TIFF fixtures and report-contract tests | Replace extension-based `is_tiff_filename()` rendered-input decisions with decoded input classification and a persisted user override. TIFF, PNG, and JPEG default to developed RGB; linear DNG remains scene RGB; only verified mosaic inputs use sensor RAW processing. |
| M6-012 | active | Reauthor colour-negative stock families from manufacturer evidence | Native role tests, `dfee_cli --stock-assay`, material-off stock assay, and visual QA | Completed slices: Vision3 250D/500T as distinct latitude-first daylight/tungsten camera negatives; Kodak Portra 160/400/800 plus Ektar 100 as differentiated professional still negatives; Kodak UltraMax 400, Gold 200, plus ColorPlus 200 consumer negatives; and Fujicolor C200, Superia X-TRA 400, Pro 400H, plus standard Eterna 250D. `COLOR_NEGATIVE_CALIBRATION.md` defines the native hue/chroma assay; the `9339116563.rw2` material-off check reports no completed-slice pair at or below `0.0100` RGB MAE. Next: portrait/reference-scan visual QA, then decide whether any stock should be retired or regrouped. |

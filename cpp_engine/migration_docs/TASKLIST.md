...
| M6-003H | in_progress | Run visual/performance acceptance probe | No-server preview timing plus image QA | First no-server timing probe is complete. Visual QA caught blotchy v2 grain in smooth skies and roughness collapsing into pixel noise; renderer now uses band-limited clumping, roughness-as-shape semantics, and artifact tests. Visual QA across representative RAWs and stock families remains before marking M6-003 complete. |
